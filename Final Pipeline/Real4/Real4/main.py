...
class EnvironmentSetup(ActionUtility):
    def __init__(self, env_name_: str,
        ...
        if not separate_policies:
            ...
            if self.terminate_indicator:
                ...
                # cube
                # best_model_path = '/Users/qiyangyan/Downloads/diffusion_imitation-master/agents/models_terminate_WORK_DONTCHANGE/SimpleNN/best_model.pth'
            ...
            if self.control_mode_indicator:
                ...
                # cube
                # best_model_path = '/Users/qiyangyan/Downloads/diffusion_imitation-master/agents/models_operating_mode_WORK_DONTCHANGE/simpleNN/best_model.pth'
    ...
    def initialize_agent(self,
        ...
        if not diffusion:
            ...
            print("Check: ", state_shape, n_actions)
...
class TrainEvaluateAgent(EnvironmentSetup):
    ...
    def play(self,
            ...
            if not real:
                if success[-1]:
                    ...
                    pos_error, orientation_error = self._goal_distance(np.array(self.env.goal[:7]),
                                                                       np.array(self.env.data.qpos[4:11]))
                    pose_error_list.append(pos_error)
                    orientation_error_list.append(orientation_error)
                    # time.sleep(2)

                    if pos_error < self.env.d_threshold and orientation_error < self.env.angle_threshold:
        ...
        if collect_demonstration:
            ...
            demonstration_dict['next_observations'] = np.vstack(demonstration_dict['next_observations']).astype(
                np.float32)
    ...
    def run_episode_E2E(self,
                        ...
                        reset=True,
                        normalise=False,
                        # dataset_4_norm_path='/Users/qiyangyan/Desktop/Diffusion/Demonstration/bigSteps_10000demos_slide'
                        ):
        episode_dict = {
            "state": [],
            "action": [],
        ...
        with open(dataset_4_norm_path_stop, 'rb') as f:
            dataset_4_norm_rot = pickle.load(f)
        goal_mean_stop = np.mean(dataset_4_norm_rot['desired_goals'], axis=0)
        goal_std_stop = np.std(dataset_4_norm_rot['desired_goals'], axis=0)
        obs_mean_stop = np.mean(dataset_4_norm_rot['observations'], axis=0)
        obs_std_stop = np.std(dataset_4_norm_rot['observations'], axis=0)
        ...
        for t in range(20):
            ...
            ''' Choose action '''
            if self.terminate_indicator is True:
                slide_terminate = self.predict_terminate(normalize(state, obs_mean_stop, obs_std_stop),
                                                         normalize(desired_goal[:-2], goal_mean_stop[:-2], goal_std_stop[:-2]))
                print("Slide complete: ", slide_terminate)

            if self.control_mode_indicator is True:
                action_control_mode = self.predict_control_mode(normalize(state, obs_mean, obs_std),
                                                                normalize(desired_goal[:-2], goal_mean, goal_std))
            ...
            if t == 0:
                ...
                state_policy_input = normalize(state, obs_mean_rot, obs_std_rot)
                desired_goal_policy_input = normalize(desired_goal[:-2], goal_mean_rot, goal_std_rot)
