...
class MLP(nn.Module):
    ...
    def __init__(self,
        ...
        print("Check: ", state_dim, action_dim, t_dim, input_dim)
        self.mid_layer = nn.Sequential(nn.Linear(input_dim, layer_dim),
                                       nn.Mish(),
        ...
        self.final_layer = nn.Linear(layer_dim, action_dim)
...
class ConsistencyTrajectoryNetwork(nn.Module):
    ...
    def forward(self, x, cond, t, s):
        ...
        x = torch.cat([x, cond, embed_s, embed_t], dim=-1) if self.cond_conditional else torch.cat(
            [x, embed_s, embed_t], dim=-1)
        return self.mlp(x)
