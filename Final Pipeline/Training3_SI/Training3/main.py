...
class EnvironmentSetup(ActionUtility):
    def __init__(self, env_name_: str, render=False):
...
class TrainEvaluateAgent(EnvironmentSetup):
    def __init__(self, env_name_, render):
        super().__init__(env_name_, render=render)
    ...
    def run_episode_with_reset(self, train_mode, randomisation=False, withRotation=False, withPause=False):
        episode_dict = {
            "state": [],
            "action": [],
            "info": [],
        ...
        if withRotation == True and info_["is_success"] == 1:
            rotation_precision = 0.005
    ...
    def eval_agent(self, randomise):
        ...
        for ep in range(10):
            print("episode: ", ep)
            episode_dict, success, episode_reward = self.run_episode_with_reset(train_mode=False,
    ...
    def play(self, num_episodes, randomise, withRotation=False, withPause=False):
...
if __name__ == "__main__":
    ...
    trainer_evaluator = TrainEvaluateAgent(env_name, True)

    # trainer_evaluator.train(randomise=True)
    # trainer_evaluator.eval_agent()
    trainer_evaluator.play(100, randomise=True, withRotation=True, withPause=True)

    # print(env.model.geom_type)
    # print(env.model.geom_size[16])  # IHM object
    # print(env.model.geom_size[21])  # target object
    # print(env.model.geom_size[16])
    # name = 'target'
    # print(trainer_evaluator.env._model_names._site_name2id["target:corner1"])
