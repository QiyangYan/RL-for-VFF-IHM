...
from gymnasium_robotics.envs.training5 import MujocoHandEnv
    ...
    class BaseManipulateEnv(HandEnvClass):
        def __init__(
            ...
            **kwargs,
            ...
            self.r_threshold = 0.01 # 0.005 is more accurate
            self.d_threshold = 0.015 # 0.01
            self.angle_threshold = 0.3 # 0.17
        ...
        def _is_success(self, achieved_goal, desired_goal):
            """
            Return True/False indicate success of the episode, conditions:
                1. success position
        ...
        def _is_success_radi(self, achieved_goal, desired_goal):
            ...
            if len(achieved_goal.shape) == 1:
                ...
                assert len(achieved_goal) == 11, f"Wrong size, check: {np.shape(achieved_goal)}"  # 9 element
                d_radi = achieved_goal[7:9] - desired_goal[7:9]
                d_pos, d_rot = self._goal_distance(achieved_goal[:7],desired_goal[:7])
                d_rot = abs(d_rot)
                d_pos = abs(d_pos)
                success = (d_pos < self.d_threshold and d_rot < self.angle_threshold).astype(np.float32)
            elif len(achieved_goal.shape) > 1:
                # print("Used by replay buffer")
                # ValueError("More than one achieved goal. Might be needed by replay buffer")
                '''train'''
                assert achieved_goal.shape[1] == 11, f"Wrong size, check: {np.shape(achieved_goal)}"  # 9 element
                d_radi = achieved_goal[:, 7:9] - desired_goal[:, 7:9]
                d_pos, d_rot = self._goal_distance(achieved_goal[:, :7], desired_goal[:, :7])
                d_rot = abs(d_rot)
                d_pos = abs(d_pos)
        ...
        def compute_reward_E2E(self, achieved_goal, goal):
            ...
            success, d_contact_radi, _, _ = self._is_success_radi(achieved_goal, goal)
            robot_qpos = self.data.qpos[1:5]
            d_relative_rot = achieved_goal[9:11] - goal[9:11]
            success_term = 5 if success else 0
            penalty = self.add_terminate_penalty(achieved_goal=achieved_goal)  # 0, -5

            # if penalty != 0:
            #     print("Penalty apply")

            # print("Check: ", get_rms(d_contact_radi), get_rms(d_relative_rot))
            # print(achieved_goal[9:11], goal[9:11])

            # print(d_contact_radi)
            # reward = - get_rms(d_contact_radi) * 20  - get_rms(d_relative_rot) / 60 + success_term + penalty
            reward = - get_rms(d_contact_radi) * 20 + success_term + penalty

            # print(get_rms(d_contact_radi) / 0.05, get_rms(d_relative_rot) / 60)
...
class MujocoManipulateEnv(get_base_manipulate_env(MujocoHandEnv)):
    def _get_achieved_goal(self):
        '''7 position element of object + 2 contact point radius'''
        ...
        if self.left_contact_idx != None and self.right_contact_idx != None:
            ...
            ''' RANDOMISATION '''
            left_contact_point_random, right_contact_point_random, object_qpos = self.randomise_object(
                left_contact_point, right_contact_point, object_qpos)
            ...
            achieved_goal_radi = np.zeros(2)

        achieved_angle_diff = np.zeros(2)
        achieved_goal = np.concatenate((object_qpos, achieved_goal_radi, achieved_angle_diff))
        assert achieved_goal.shape == (11,)
    ...
    def _sample_goal_random(self):
        """
        Randomly sample a goal pose
        """
        ...
        '''
        goal_radi = np.zeros(2) # this is because the radius is computed based on site, this can be obtained after render callback
    ...
    def _sample_goal(self):
        """
        For simplicity of the task, use a fixed goal
        """
        # print("Check")
        # pose = np.array([ 2.1969790e-02,-2.9955113e-01, 0, 0.9063078, 0, 0, 0.4226183])
        pose = np.array([ 0.00853086, -0.30000001, 0.002, 0.99941308, 0., 0., 0.03425628])
        # goal_radi = self.compute_goal_radi(pose[:3], pose[:3])
        goal_radi = np.zeros(2)
        goal_relative_rot = np.zeros(2)
        goal = np.concatenate([pose, goal_radi, goal_relative_rot])
        assert len(goal) == 11, f"Wrong goal size, check: {goal}"
        return goal
    ...
    def _get_contact_point(self):
        ...
        # print("Check", self.left_contact_idx)
        ...
        self.compute_finger_angle(left_contact_coord[:3], right_contact_coord[:3])
    ...
    def compute_finger_angle(self, left_contact_coord, right_contact_ccord, left_motor=[0.037012, -0.1845, 0.002], right_motor=[-0.037488, -0.1845, 0.002]):
        # Calculate joint offset based on the finger width
        finger_width = 0.034
        finger_offset_l = np.rad2deg(np.arcsin(finger_width / self.goal_radi[0]))
        finger_offset_r = np.rad2deg(np.arcsin(finger_width / self.goal_radi[1]))

        # Compute finger goal angle (angle between y-axis and the line cross contact point and actuator)
        dx_l = left_contact_coord[0] - left_motor[0]
        dy_l = left_contact_coord[1] - left_motor[1]

        # print("Check:  ", np.rad2deg(np.arctan(dx_l/dy_l)), finger_offset_l)
        finger_angle_l = 180 + np.rad2deg(np.arctan(dx_l/dy_l)) - finger_offset_l

        dx_r = right_contact_ccord[0] - right_motor[0]
        dy_r = right_contact_ccord[1] - right_motor[1]

        # print(dx_r, dy_r)

        # print("Check 2: ", np.rad2deg(np.arctan(dx_r/dy_r)), finger_offset_r)
        finger_angle_r = 180 + np.rad2deg(np.arctan(dx_r/dy_r)) + finger_offset_r
        # print("Check: ", finger_angle_l, finger_angle_r)

        self.goal_finger_angle = [finger_angle_l, finger_angle_r]
    ...
    def _render_callback(self):
        ...
        self.goal[7:9] = self.goal_radi

    def compute_finger_relative_angle_diff(self, desired_goal, achieved_goal, robot_qpos):
        """
        finger_relative_angle_diff = achieved goal angle - finger angle
        """
        assert len(desired_goal) == 11 and len(achieved_goal) == 11, f"Check: {len(desired_goal), len(achieved_goal)}"

        angle_diff = 0
        goal_quat_angle = desired_goal[3:7]
        goal_euler_angles = R.from_quat(goal_quat_angle).as_euler('xyz', degrees=True)[0]
        goal_euler_angles = np.mod(goal_euler_angles, 360)

        achieved_goal_quat_angle = achieved_goal[3:7]
        achieved_goal_euler_angles = R.from_quat(achieved_goal_quat_angle).as_euler('xyz', degrees=True)[0]
        achieved_goal_euler_angles = np.mod(achieved_goal_euler_angles, 360)

        left_finger_angle = robot_qpos[0] / 1.8807 * 107.754 + 127.002
        right_finger_angle = 360 - ((robot_qpos[2] / 1.8807 * 107.754) + 127.002)

        # achieved angle - finger angle
        self.angle_diff = np.array(
            [achieved_goal_euler_angles - left_finger_angle, achieved_goal_euler_angles - right_finger_angle])
        self.goal_angle_diff = np.array(
            [goal_euler_angles - self.goal_finger_angle[0], goal_euler_angles - self.goal_finger_angle[1]])

        # print(angle_diff, goal_angle_diff, goal_euler_angles, achieved_goal_euler_angles)

        # print(goal_euler_angles, self.goal_finger_angle)
        # print(self.goal_angle_diff)
        # print(self.angle_diff)

        return self.angle_diff - self.goal_angle_diff
    ...
    def _get_obs(self):
        ...
        d_contact_radi = np.zeros(2)
        d_relative_rot = np.zeros(2)
        if len(self.goal.ravel()) == 0:
            radi_diff = np.zeros(2)
            angle_diff = 0
        else:
            assert len(self.goal.ravel()) == 11 and len(
                achieved_goal) == 11, f"Wrong goal length, check: {self.goal.ravel(), achieved_goal}"
            d_contact_radi = self.goal.ravel()[7:9] - achieved_goal[7:9]
            d_relative_rot = self.compute_finger_relative_angle_diff(desired_goal, achieved_goal, robot_qpos)
            achieved_goal[9:11] = self.angle_diff
            desired_goal[9:11] = self.goal_angle_diff
            self.goal[9:11] = self.goal_angle_diff
        ...
        observation = np.concatenate(
            [
                ...
                d_contact_radi, # (2, ) radius difference (radi = object centre to actuators)
                d_relative_rot, # (2, ) orientation difference
            ]
        )
        ...
        complete_obs = {
            ...
            # (9, ) pose (7,) + achieved radi (2, ) + achieved relative angle (2, )
            "achieved_goal": achieved_goal.copy(),
            # (9, ) pose (7,) + goal radi (2, ) + goal relative angle (2, )
        ...
        # assert len(obs['observation']) == 25, f"Check: {obs['observation']}"  # 4 + 4 + 7 + 6 + 2 + 2 = 25
        # assert len(obs['achieved_goal']) == 11, f"Check: {obs['achieved_goal']}"  # 7 + 2 = 9
        # assert len(obs['desired_goal']) == 11 or len(obs['desired_goal']) == 0, f"Check: {obs['desired_goal']}"  # 7 + 2 = 9
    ...
    def compute_terminated(self, achieved_goal, desired_goal, info):
        # exceed range
        assert len(achieved_goal) == 11, f"achieved goal should have length of 9, check: {achieved_goal}"
