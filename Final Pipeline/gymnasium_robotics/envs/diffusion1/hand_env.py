...
# def remove_z_axis_rotation(quaternion):
#     # Step 1: Convert quaternion to Euler angles
#     r = R.from_quat(quaternion)
#     roll, pitch, yaw = r.as_euler('xyz', degrees=False)
#
#     # Step 2: Set yaw to zero to remove rotation about Z-axis
#     yaw = 0
#
#     # Step 3: Convert back to quaternion
#     r_adjusted = R.from_euler('xyz', [roll, pitch, yaw], degrees=False)
#     quaternion_adjusted = r_adjusted.as_quat()
#
#     return quaternion_adjusted
...
class MujocoHandEnv(get_base_hand_env(MujocoRobotEnv)):
    ...
    def get_pos_ctrl_params(self, randomise=False):
        if randomise == False:
            ...
            self.pos_ctrl_params = {
                'damping': self.randomise.uniform_randomise("damping"),
                'armature': self.randomise.uniform_randomise("armature"),
                'frictionloss': 0.03,
                'ctrlrange': [0, 1.8807],
                'biastype': 1,
                'kp': self.randomise.uniform_randomise("kp"),
                'floor_friction': 0.01
            }

    def get_torque_ctrl_params(self):
    ...
    def _set_action(self, action=None):
        ...
        if isinstance(action, np.int64):
            ...
            gripper_pos_ctrl = (0, 1.8807)
            self.friction_state = (1, -1)
            if control mode has different friction state as the current position
            '''
            step_friction_state, control_mode = self.discretize_action_to_control_mode_E2E(action[1])
            # print("hand_env", step_friction_state, control_mode, action[1])
            if step_friction_state != self.last_friction_state:
                self.friction_change_penalty = True
            else:
                self.friction_change_penalty = False

            _, pos_idx = self.action_to_control_E2E(control_mode)
            if control_mode == 0 or control_mode == 1 or control_mode == 2 or control_mode == 3:
                self.sliding = True
            else:
                self.sliding = False
            # print(pos_idx)

            # print("pos_idx", pos_idx, self.pos_idx)
            ...
            if (action[0] + 1) / 2  != self.gripper_pos_ctrl:
                ...
                self.gripper_pos_ctrl = (action[0] + 1) / 2 # action is the movement related to the current position, 0 ~ 1
                # self.move = 0
            ...
            if self.firstEpisode:
                ...
                self.data.ctrl[1 - self.pos_idx] = 0.3
                # TODO: Uncomment below for VFF-bigSteps
                # print("Step size: ", action[0])
                # if control_mode == 4 or control_mode == 5:
                #     self.data.ctrl[self.pos_idx] = self.start - action[0]  # rotation part is rad directly
                # else:
                #     self.data.ctrl[self.pos_idx] = self.start - self.gripper_pos_ctrl * 1.8807 / 7  # sliding is as original

                # TODO: Uncomment below for VFF-bigSteps data collection
                self.data.ctrl[self.pos_idx] = self.start - self.gripper_pos_ctrl * 1.8807 / 7
                # print(self.gripper_pos_ctrl * 1.8807 / 7)

                # TODO: Uncomment below for random policy VFF-bigSteps data collection
                # self.data.ctrl[self.pos_idx] = self.start - self.gripper_pos_ctrl * 1.8807
                # print("Move: ", self.gripper_pos_ctrl * 1.8807)

            # print(self.pos_idx)
            # print(self.gripper_pos_ctrl)

        elif len(action) == 7:  # synchronize sim object pose with actual object pose:

            # pose = remove_z_axis_rotation(action)
            rotation = R.from_quat(action[3:])
            euler = rotation.as_euler('xyz', degrees=True)
            # print(euler)
            rotation = R.from_euler('xyz', np.array([-90-euler[2],0,0]), degrees=True)
            quat = rotation.as_quat()
            action = np.concatenate([action[:3], quat])
            action += [0, -0.10225, 0, 0, 0, 0, 0]
            self._utils.set_joint_qpos(self.model, self.data, "joint:object", action)
            self._utils.set_joint_qvel(self.model, self.data, "joint:object", np.zeros(6))
    ...
    def discretize_action_to_control_mode_E2E(self, action):
    ...
    def _set_action_continuous(self, action):
        ...
        if friction_state == 2:
            ...
            if self.pick_up == False:
                ...
                self.data.ctrl[1] = 0
                self.data.ctrl[4] = 0
                # if abs(self.data.qpos[1] - 1.05) < 0.002: # when hh, 0.001
                if abs(self.data.qpos[1] - 0.9557) < 0.003:  # when hh, 0.001
                    self.pick_up = True
                ...
                if self.count == 49:
                    self.closing = True
                # print(self.count, self.closing, self.pick_up)
    ...
    def switch_ctrl_type_pos_idx(self, pos_idx):
        ...
        self.model.actuator_gainprm[self.pos_idx][0] = self.pos_ctrl_params['kp']
        self.model.actuator_biastype[self.pos_idx] = self.pos_ctrl_params['biastype']
        self.model.actuator_biasprm[self.pos_idx][1] = -self.pos_ctrl_params['kp']
        ...
        self.data.ctrl[self.torque_idx] = 0.3
    ...
    def check_physics_parameter(self):
        print("Damping: ", self.model.dof_damping[self.pos_idx * 2 + 1])
        print("Armature: ", self.model.dof_armature[self.pos_idx * 2 + 1])
        print("Frictionless: ", self.model.dof_frictionloss[self.pos_idx * 2 + 1])
        print("Gainprm: ", self.model.actuator_gainprm[self.pos_idx][0])
        print("Biasprm: ", self.model.actuator_biasprm[self.pos_idx][0])
        print("Force Range: ", self.model.actuator_forcerange[self.pos_idx])
        print("Floor Friction: ", self.model.geom_friction[1][0])
        # print("Object Friction: ", self.env.model.geom_friction[16][0])
        # for i, geom_name in enumerate(self.env.unwrapped.model.geom_names):
        #     friction = self.env.unwrapped.model.geom_friction[i]
        #     print(f"Friction for {geom_name}: {friction}")
    ...
    def action_to_control_E2E(self, action):
        """
        Action Space:
        Action  |   Movement                        |   friction state      |   pos index
        0           Slide up on right finger                1                       0
        ...
        if action == 0:
            ...
            assert action == 5, "action value outputted by policy is incorrect"
            friction_state = 0
            pos_idx = 1
        return friction_state, pos_idx
