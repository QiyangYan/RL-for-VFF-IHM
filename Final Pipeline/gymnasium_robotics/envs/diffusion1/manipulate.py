...
from gymnasium_robotics.envs.diffusion1 import MujocoHandEnv
    ...
    class BaseManipulateEnv(HandEnvClass):
        def __init__(
            ...
            **kwargs,
            ...
            self.d_threshold = 0.005 # 0.01
            self.angle_threshold = 0.1 # 0.17
            ...
            self.plot_reward_history = False
            self.reward_history = {
                "d_radi": [],
                "d_relative_angle": [],
                "d_rot": [],
        ...
        def randomise_physics_params(self):
            ...
            if randomise:
                ...
                print(f"\033[92m | Randomise physics param \033[0m")
            else:
                print("\033[91m ------------------------------------------- \033[0m")
                print(f"\033[91m | No Randomise physics param \033[0m")
        ...
        def compute_reward(self, achieved_goal, goal, info):
            ...
            if self.reward_type == "sparse":
                ...
                if len(goal) == 11:
                    ...
                    # print(np.mean(abs(d_contact_radi)))
                else:
                ...
                # print("d_radi: ", np.mean(abs(d_radi)) * 20)
...
class MujocoManipulateEnv(get_base_manipulate_env(MujocoHandEnv)):
    def _get_achieved_goal(self):
        ...
        if self.left_contact_idx != None and self.right_contact_idx != None:
            ...
            self.achieved_goal_contact_point_radi = self.compute_goal_radi(left_contact_point, right_contact_point)
            # print("contact points:",self.left_contact_idx,self.right_contact_idx, achieved_goal_radi)
        else:
            self.achieved_goal_contact_point_radi = [0,0]
            # print("initialising")
    ...
    def _reset_sim(self):
        ...
        # Create the plot
        if self.reward_history['d_radi'] and self.plot_reward_history == True:
            plt.figure(figsize=(10, 5))
            plt.plot(self.reward_history["d_radi"], label='Contact point radius', color='r')
            plt.plot(self.reward_history['d_relative_angle'], label='Finger relative angle', color='b')
            plt.plot(self.reward_history['d_radi_centre'], label='Object centre radius', color='orange')
            plt.plot(self.reward_history['d_rot'], label='Orientational Difference', color='lightskyblue')
            plt.plot(self.reward_history['d_pos'], label='Positional difference', color='lightgreen')
    ...
    def _sample_goal(self):
        ...
        # mid_goals = load_pickle("/Users/qiyangyan/Desktop/TrainingFiles/Diffusion1/demonstration/bigSteps_10k_demos_random_test_goals")
 ...
 #  1.15151313e-01])
    ...
    def compute_finger_angle(self, left_contact_coord, right_contact_ccord, left_motor=[0.037012, -0.1845, 0.002], right_motor=[-0.037488, -0.1845, 0.002]):
        ...
        # print(np.rad2deg(np.arctan(dx_l/dy_l)))
        ...
        # print(np.rad2deg(np.arctan(dx_r/dy_r)))
    ...
    def compute_finger_relative_angle_diff(self, desired_goal, achieved_goal, robot_qpos):
        ...
        # print(angle_diff, goal_angle_diff, get_rms(angle_diff-goal_angle_diff))
        # print(left_finger_angle, right_finger_angle)

        return angle_diff - goal_angle_diff
    ...
    def _get_obs(self):
        ...
        if len(self.goal.ravel()) != 0 and self.closing == True and self.pick_up == True and self.plot_reward_history == True:
            d_relative_rot = get_rms(self.compute_finger_relative_angle_diff(desired_goal, achieved_goal, robot_qpos))
            d_contact_radi = get_rms(self.achieved_goal_contact_point_radi - desired_goal[7:9])
            d_pos, d_rot = self._goal_distance(achieved_goal[:7], desired_goal[:7])
            self.reward_history['d_radi'].append(d_contact_radi*20)
            self.reward_history['d_relative_angle'].append(d_relative_rot/60)
            self.reward_history['d_rot'].append(abs(d_rot))
            self.reward_history['d_radi_centre'].append(np.mean(abs(radi_diff/0.05)))
            self.reward_history['d_pos'].append(abs(d_pos/0.05))
