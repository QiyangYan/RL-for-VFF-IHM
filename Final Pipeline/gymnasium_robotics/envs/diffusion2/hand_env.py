...
class MujocoHandEnv(get_base_hand_env(MujocoRobotEnv)):
    def __init__(
        ...
        self.step_size_limit = 1.8807 / 10  # 1
    ...
    def _set_action(self, action=None):
        ...
        if isinstance(action, np.int64):
            ...
            # TODO:
            #  Uncomment 1: This is for random policy with acceleration and deceleration
            #  Uncomment 2: This is for random policy with big steps
            ' -1- '
            # self.gripper_pos_ctrl = np.clip(((action[0] + 1) / 2) * self.step_size_limit, 0.005, self.step_size_limit)  # 1
            ' -2- '
            self.gripper_pos_ctrl = (action[0] + 1) / 2 * 1.8807
            ...
            if self.firstEpisode:
                ...
                if control_mode == 4 or control_mode == 5:
                    '''old'''
                    # self.data.ctrl[self.pos_idx] = self.start - self.gripper_pos_ctrl * 1.8807 / 2
                    self.data.ctrl[self.pos_idx] = self.data.qpos[self.pos_idx*2+1] - self.gripper_pos_ctrl
                else:
                    '''old'''
