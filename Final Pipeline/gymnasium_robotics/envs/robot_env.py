...
class BaseRobotEnv(GoalEnv):
    ...
    def step(self, action):
        ...
        #     "is_success_old": self._is_success_old(obs["achieved_goal"], self.goal),
        # }

        # TODO: Uncomment below for using RL policy for data collection, only sliding evaluation
        info = {
            "is_success": self._is_success_old(obs["achieved_goal"], self.goal), # old is radi
            "is_success_old": self._is_success(obs["achieved_goal"], self.goal),
        }
        '--------'

        # TODO: Uncomment below for E2E evaluation
        # info = {
        #     "is_success": self._is_success(obs["achieved_goal"], self.goal),
        #     "is_success_old": self._is_success_old(obs["achieved_goal"], self.goal),  # old is radi
        # }

        # TODO: Uncomment below for training5, and automate terminator, play
        # info = {
        #     "is_success": self._is_success(obs["achieved_goal"], self.goal),
        # }
        '--------'
    ...
    def step_real_process(self, obs):
        self._render_callback()
    ...
    def reset(
        ...
        *,
        ...
        if options is None:
            ...
            if options["repeat_same_goal"] is True:
                pass
            else:
                self.goal = self._sample_goal().copy()
