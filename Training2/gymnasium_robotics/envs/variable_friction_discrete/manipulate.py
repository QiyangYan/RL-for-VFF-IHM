...
from gymnasium_robotics.envs.variable_friction_discrete import MujocoHandEnv
    ...
    class BaseManipulateEnv(HandEnvClass):
        def __init__(
            ...
            **kwargs,
            ...
            self.terminate_r_limit = [0.035,0.14]
            self.L = 0.015
            self.success = False
        ...
        def compute_reward(self, achieved_goal, goal, info):
            ...
            if self.reward_type == "sparse":
                ...
                if len(goal) == 9:
                    ...
                    assert len(goal) == 2, f"The length of goal is wrong, check: {len(goal)}"
                ...
                if self.pick_up_height == 3:
                    ...
                    reward_dict["RL_IHM"] = - np.mean(abs(d_radi)) * 20 - self.slip_error * 20000 - self.slip_error_angle * 20000
                else:
                    'with support'
                    assert self.pick_up_height == 0
                    reward_dict["RL_IHM"] = - np.mean(abs(d_radi)) * 20 + friction_change_penalty + success

                'RL_based_Friction'
                'without support'
                if self.pick_up_height == 3:
                    reward_dict["RL_inspired_IHM_with_RL_Friction"] = - self.friction_change_error * 20000 - self.slip_error * 20000 - self.slip_error_angle * 20000
                else:
                    'with support'
                    assert self.pick_up_height == 0
                    reward_dict["RL_inspired_IHM_with_RL_Friction"] = - self.friction_change_error * 20000
...
class MujocoManipulateEnv(get_base_manipulate_env(MujocoHandEnv)):
    ...
    def compute_terminated_radi(self, radius, info):
        # exceed range
        radius_l, radius_r = radius[0], radius[1]
        # radius_l, radius_r = self._compute_radi(achieved_goal[:7])
        # print("compute terminated", z)
        # radius_l = achieved_goal[7]
        # radius_r = achieved_goal[8]
