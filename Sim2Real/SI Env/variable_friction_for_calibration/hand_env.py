...
class MujocoHandEnv(get_base_hand_env(MujocoRobotEnv)):
    ...
    def _set_action(self, action=None):
        ...
        self.data.ctrl[0] = action  # left:0, right:1
        # print(ctrlrange[:, 0], ctrlrange[:, 1])
    ...
    def _set_action_continuous(self, action):
        ...
        if friction_state == 2:
            ...
            if self.pick_up == False:
                ...
                self.data.ctrl[1] = 0
                self.data.ctrl[4] = 0
                if abs(self.data.qpos[1] - 1.05) < 0.01: # when hh, 0.001
                    self.pick_up = True
            ...
            # print(self.data.ctrl[0], self.data.ctrl[1])
            ...
            if self.firstEpisode:
                ...
                # rotation
                if action[0] == 1: # rotation
                    if self.rotate_start == False:
                        self.rotate_start = True
                        self.pos_idx = self.get_pos_idx_for_rotation()
                        self.switch_ctrl_type_pos_idx(self.pos_idx)
                    self.data.ctrl[1-self.pos_idx] = 0.5
                    self.data.ctrl[self.pos_idx] = self.data.ctrl[self.pos_idx] - 0.01
                elif action[0] == -1: # anti-rotation
                    if self.rotate_start == True:
                        self.rotate_start = False
                        self.pos_idx = 1 - self.pos_idx # switch position control for anti-rotation
                        self.switch_ctrl_type_pos_idx(self.pos_idx)
                    self.data.ctrl[1 - self.pos_idx] = 0.5
                    self.data.ctrl[self.pos_idx] = self.data.ctrl[self.pos_idx] - 0.01
                else: # stablise finger in case the ctrl is too much
                    assert action[0] == 0, "action[0] should be either 1, 0 or -1 for rotation part, -1 for anti-rotation"
    ...
    def get_pos_idx_for_rotation(self):
    ...
    def switch_ctrl_type_pos_idx(self, pos_idx):
        ...
        self.model.actuator_ctrlrange[self.torque_idx] = [0, 1]
        ...
        self.model.actuator_ctrlrange[self.pos_idx] = self.pos_param[3] # 0~1.68
    ...
    def switch_ctrl_type_friction(self,next_friction_state):
        ...
        self.model.actuator_ctrlrange[1-self.pos_idx] = [0, 1]

        self.model.actuator_gainprm[self.pos_idx][0] = self.pos_param[0]
        self.model.actuator_biastype[self.pos_idx] = self.pos_param[1]
        self.model.actuator_biasprm[self.pos_idx][1] = self.pos_param[2]
        self.model.actuator_ctrlrange[self.pos_idx] = self.pos_param[3]
