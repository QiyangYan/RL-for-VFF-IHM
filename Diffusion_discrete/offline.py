...
def train_agent(env, env_name, state_dim, action_dim, max_action, device, output_dir, writer, args):
    ...
    elif args.model == 'consistency_trajectory':
        from agents.ql_consistency_trajectory import ConsistencyTrajectory_QL as Agent
        agent = Agent(state_dim=state_dim,
                    action_dim=action_dim,
                    max_action=max_action,
                    device=device,
...
def eval_policy(policy, env_name, seed, eval_episodes=10):
    ...
    try:
        policy.model.eval()
    except:  # CTM has a bit different structure
        policy.actor.model.eval()
    policy.actor.eval()
...
if __name__ == "__main__":
    ...
    parser.add_argument("--dir", default="ctm_results", type=str)                    # Logging directory
    ...
    parser.add_argument("--model", default="diffusion", type=str)  # ['diffusion', 'consistency', 'consistency_trajectory']
