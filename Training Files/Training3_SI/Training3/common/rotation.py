...
class ROTATION:  # this class contains functions for rotation
    ...
    def start_rotation(self, action):
        ...
        self.rotation_start_pos = reward_dict["torque_control_position"]

        # last_reward = reward
        reward = reward_dict["d_pos"] * 20
        start_reward = reward
        last_reward = reward
        ...
        while reward <= last_reward:
            ...
            if terminated:
                return terminated, reward_dict
            last_reward = reward_dict["d_pos"]
            reward = reward_dict["d_pos"]

        action[0] = reward_dict["pos_control_position"]
        next_env_dict, distance, terminated, truncated, info = self.env.step(np.array(action))
        if terminated is True:
            reward = distance["d_pos"] * 20
            return terminated, reward_dict
        action[0] = reward_dict["torque_control_position"]
