...
class MujocoHandEnv(get_base_hand_env(MujocoRobotEnv)):
    ...
    def _set_action(self, action=None):
        ...
        if isinstance(action, np.int64):
            ...
            step_friction_state, self.pos_idx = self.action_to_control(action)
        ...
        elif self.IHM_RL and len(action) == 2:
            '''IHM with continuous action space'''
            raise Exception("This is not implemented yet", action, len(action))
    ...
    def _set_action_friction(self, action):
        ...
        if action[1] == 1 or action[1] == -1 or action[1] == 0:
            # torque
            gripper_torque_ctrl, friction_state = action[0], action[1]
            ctrlrange = self.model.actuator_ctrlrange
    ...
    def check_action_complete(self):
        ...
        if friction_check_1 and friction_check_2 and abs(self.motor_diff) < motor_limit:
            # print("friction diff: ", friction_diff1, friction_diff2, abs(self.motor_diff))
            # print(self.data.ctrl[2], self.data.ctrl[3], self.data.qpos[2], self.data.qpos[4])
            return True
        else:
            return False
    ...
    def friction_complete_check(self, ctrl_val, qpos):
        if ctrl_val == 0:  # low friction
            if abs(qpos) < 0.00165:  # this is the boundary the friction turns from low to high
                return True
        else:
