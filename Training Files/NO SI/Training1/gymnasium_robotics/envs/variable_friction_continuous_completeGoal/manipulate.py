...
from gymnasium_robotics.envs.variable_friction_continuous_completeGoal import MujocoHandEnv
    ...
    class BaseManipulateEnv(HandEnvClass):
        def __init__(
            ...
            **kwargs,
            ...
            self.r_threshold = 0.01 # 0.001 is more accurate
        ...
        def compute_reward(self, achieved_goal, goal, info):
            ...
            if self.reward_type == "sparse":
                ...
                if len(achieved_goal) == 2 and achieved_goal.ndim == 0:
                    ''' 
                    Used: in the step()
                    Input Shape: self.goal, with length = (3+4+2)
                    achieved_goal, with length 2, two radius
                    '''
                    assert goal.ndim == 0, f"goal has wrong dimension: {goal.ndim}"
                    assert len(goal) == 9, f"goal has wrong length: {len(goal)}"
                    success_radi, success_pos, d_radi, d_pos = self._is_success_radi(achieved_goal, goal[7:])

                elif len(achieved_goal) == 5 and achieved_goal.ndim == 0:
                    ''' 
                    Used: in the step()
                    Input Shape: self.goal, with length = (3+4+2)
                    achieved_goal, with length 5, 2 + 3 = radi + object pos
                    '''
                    assert goal.ndim == 0, f"goal has wrong dimension: {goal.ndim}"
                    assert len(goal) == 9, f"goal has wrong length: {len(goal)}"
                    success_radi, success_pos, d_radi, d_pos = self._is_success_radi(achieved_goal[:2], goal[7:])

                elif goal.shape == (256, 2):
                    ''' 
                    Used: in the replay buffer
                    Input Shape: obs["desired_goal"] stored in the buffer, with shape = (256, 2)
                    Note: This is used when the achieved goal is only the radius with length = 2
                    '''
                    assert achieved_goal.shape == (256, 2), f"The shape of goal is wrong, check: {achieved_goal.shape}"
                ...
                elif goal.shape == (256, 5):
                    ''' 
                    Used: in the replay buffer
                    Input Shape: obs["desired_goal"] stored in the buffer, with shape = (256, 5)
                    Note: This is used when the achieved goal is the radius and pos with length = (2+3)
                    '''
                    assert achieved_goal.shape == (256, 5), f"The shape of goal is wrong, check: {achieved_goal.shape}"
                    ...
                    raise ValueError(f"The shape of goal or achieved goal is incorrect: {achieved_goal.shape, goal.shape}")
...
class MujocoManipulateEnv(get_base_manipulate_env(MujocoHandEnv)):
    ...
    def _get_obs(self):
        ...
        obs = {
            "observation": complete_obs["observation"].copy(),
            "achieved_goal": np.concatenate([achieved_goal_last_two, complete_obs["achieved_goal"][:3]]),
            "desired_goal": np.concatenate([desired_goal_last_two, complete_obs["desired_goal"][:3]]),
        }

        assert len(obs["achieved_goal"]) == 5, "achieved goal of obs has wrong shape"
        assert len(obs["desired_goal"]) == 5, "desired goal of obs has wrong shape"
        assert len(obs["observation"]) == 10, "observation element of obs has wrong shape"
