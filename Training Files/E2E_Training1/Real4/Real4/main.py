...
class EnvironmentSetup(ActionUtility):
    def __init__(self, env_name_: str, render=False, real=False, display=False, diffusion=False):
        self.goal_std = None
        self.goal_mean = None
        ...
        # print('3')
        self.agent = self.initialize_agent(diffusion=diffusion)
        if real:
    ...
    def initialize_agent(self, diffusion=False):
...
class TrainEvaluateAgent(EnvironmentSetup):
    def __init__(self, env_name_, render, real, display, diffusion=False):
        super().__init__(env_name_, render=render, real=real, display=display, diffusion=diffusion)
        # print('2')
    ...
    def run_episode(self, train_mode, randomisation=False, withRotation=False, withPause=False, real=False, display=False, reset=True, collect_demonstration=False, success_threshold=0.003):
        episode_dict = {
            "state": [],
            "action": [],
            "info": [],
        ...
        for t in range(20):
            if self.diffusion:
                # desired_goal = normalize(self.env.goal[:7], self.goal_mean, self.goal_std)
                desired_goal = self.env.goal[:7]
            if t == 0:
                action, state_norm, desired_goal_norm = self.agent.choose_action(state, desired_goal, train_mode=train_mode)
            ...
            if collect_demonstration:
                episode_dict["state"].append(state_norm[0][:8].copy())  # original obs contains radi info, don't want that
    ...
    def calculate_norm_parameter(self):
        pickle_file_path = '/Users/qiyangyan/Desktop/Training Files/Real4/Real4/demonstration/VFF-1686demos'
        data = self.load_pickle(pickle_file_path)
        true_indices = [index for index, value in enumerate(data['terminals']) if value]
        print("Check: ", true_indices)
        print(len(true_indices)/2)

        episode_start_index = 0
        for cycle in range(int(len(true_indices)/2)):
            mb = []
            for episode in range(2):
                ...
                episode_end_index = true_indices[cycle*2+episode] + 1
                if (cycle*2+episode) == 0:
                    episode_start_index = 0
                else:
                    episode_start_index = true_indices[cycle*2+episode-1]+1
                # print(cycle, episode_start_index, episode_end_index)
                # episode_dict = {key: value[episode_start_index:episode_end_index] for key, value in data.items()}
                episode_dict['state'] = data['observations'][episode_start_index:episode_end_index]
                episode_dict['desired_goal'] = data['desired_goals'][episode_start_index:episode_end_index]
                episode_dict['achieved_goal'] = data['desired_goals_radi'][episode_start_index:episode_end_index]
                episode_dict['desired_goal_radi'] = data['desired_goals_radi'][episode_start_index:episode_end_index]
                episode_dict['action'] = data['actions'][episode_start_index:episode_end_index]
                episode_dict['next_state'] = data['next_observations'][episode_start_index:episode_end_index]
                episode_dict['reward'] = data['rewards'][episode_start_index:episode_end_index]
                episode_dict['terminals'] = data['terminals'][episode_start_index:episode_end_index]
                mb.append(dc(episode_dict))
            print("store")
            self.agent.store(mb)
        self.agent.save_weights('norm_param')

    def run_episode_E2E(self, train_mode, randomisation=False, withPause=False, real=False, reset=True, display=False, success_threshold=0.003):
        episode_dict = {
            "state": [],
            "action": [],
            "info": [],
        ...
        for t in range(50):
            ...
            if t == 0:
                action, state_norm, desired_goal_norm = self.agent.choose_action(state, desired_goal,
                                                                                 train_mode=train_mode)
                friction_state, control_mode = self.discretize_action_to_control_mode_E2E(action[1])
            elif real:
                action, friction_state, control_mode, state_norm, desired_goal_norm = \
                    self.choose_action_with_filter_real(state,
                                                        desired_goal,
                ...
                action, friction_state, control_mode, state_norm, desired_goal_norm = \
                    self.choose_action_with_filter_E2E(state,
                                                   desired_goal,
                                                   self.r_dict,
                                                   t,
                                                   control_mode,
            ...
            if not train_mode:
                print("Action: ", (action[0]+1)/2, control_mode)
                # print("Reward: ", r)
            # if friction_state == 0:
            # input(f"press_to_continue: {action[0], control_mode}")
            # time.sleep(0.5)
            episode_dict["state"].append(state.copy())
            episode_dict["action"].append(action.copy())
            episode_dict["achieved_goal"].append(achieved_goal.copy())
    ...
    def choose_action_with_filter_E2E(self, state, desired_goal, reward_dict, t, control_mode, train_mode):
        action, state_norm, desired_goal_norm = self.agent.choose_action(state, desired_goal, train_mode=train_mode)
        ''' Action feasibility filter '''
        # print(reward_dict)
        if reward_dict["pos_control_position"] <= 0.03 \
            ...
            while True:
                ...
                if i > 10:
                    action[1] = np.random.uniform(-1, 1)
                friction_state, new_control_mode = self.discretize_action_to_control_mode_E2E(action[1])
                # print("Check: ", new_control_mode, control_mode)
                if control_mode == new_control_mode \
                        or (control_mode == 0 and new_control_mode == 3) \
                        or (control_mode == 1 and new_control_mode == 2) \
