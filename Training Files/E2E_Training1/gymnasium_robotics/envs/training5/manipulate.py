    ...
    class BaseManipulateEnv(HandEnvClass):
        ...
        def _is_success(self, achieved_goal, desired_goal):
            ...
            if len(achieved_goal.shape) == 1:
                ...
                d_rot = abs(d_rot)
                d_pos = abs(d_pos)
                success = d_pos < self.d_threshold and d_rot < self.d_threshold
        ...
        def compute_reward(self, achieved_goal, goal, info):
            ...
            if self.reward_type == "sparse":
                ...
                if len(goal) == 11:
                    ...
                    assert goal.shape == (2048, 11), f"The shape of goal is wrong, check: {goal.shape}"
                    success, d_radi, d_rot, d_pos = self._is_success_radi(achieved_goal, goal)
        ...
        def compute_reward_E2E_buffer(self, achieved_goals, goals):
            ...
            assert np.array(reward).shape[0] == 2048, f"Wrong shape, check: {np.shape(reward)}"
...
class MujocoManipulateEnv(get_base_manipulate_env(MujocoHandEnv)):
    ...
    def _sample_goal(self):
        pose = np.array([ 2.1969790e-02,-2.9955113e-01, 0, 0.258819, 0, 0,  0.9659258])
