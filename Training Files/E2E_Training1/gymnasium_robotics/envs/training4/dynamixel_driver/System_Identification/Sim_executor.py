...
class MuJoCo_Simulation_executor():
    ...
    def manual_policy(self, ID, goal_pos_list_dyn, real_time_):
        """
        ...
        # T = round(round(real_time / self.env.dt) + 5)
        obs, info = self.env.reset()
        start_pos = AngleConvert.xm_2_rad(abs(self.MAX_POS[ID]-self.MIN_POS[ID]))
        ...
        # if goal_pos_list_dyn is None:
        #     # calibration trajectory
        #     goal_pos_list_dyn = [1, 10, 100, 10, 10, 10, 200, 245, 10, 40, 50, 140, 245, 245, 10, 10, 10]
        #     # IHM trajectory
        #     # goal_pos_list_dyn = [245, 200, 245, 200, 245]
        #     # More
        #     # goal_pos_list_dyn = [1, 10, 100, 10, 10, 10, 200, 245, 10, 40, 50, 140, 245, 245, 10, 10, 10, 245, 200, 245, 200, 245]
        ...
        for i, pos in enumerate(goal_pos_list):
            ...
            # print(current_pos + pos * (ID * 2 - 1), current_pos, pos)
            for _ in range(11):
                step += 1
                ...
                current_pos = obs["observation"][ID * 2]
                t.append(t1)
                control.append(AngleConvert.rad_2_xm(min + goal_pos*(ID*-2+1)))
                # print(control[-1])
                # print(AngleConvert.rad_2_xm(current_pos * (ID * -2 + 1)))
                history.append(AngleConvert.rad_2_xm(min + current_pos*(ID*-2+1)))
                vel_history.append(AngleConvert.xm_rad_per_sec_to_rpm(-obs["observation"][ID * 2 + 4] * (ID * 2 - 1)))
            if t1 > real_time_:
                break
    ...
    def adjust_parameter_left(self, damping=1.084, armature=0.045, frictionloss=0.03, gainprm=21.1, biastype=1, gear=1, torque=False):
        ...
        # self.env.unwrapped.model.actuator_ctrlrange[0] = [0, 10]
        # print(self.env.unwrapped.model.actuator_ctrlrange[0])
        # self.env.unwrapped.model.actuator_ctrlrange = [0, 1]

        # TODO: place torque control in here
    ...
    def check_parameter_left(self):
        print("Damping: ", self.env.unwrapped.model.dof_damping[1])
        print("Armature: ", self.env.unwrapped.model.dof_armature[1])
        print("Frictionless: ", self.env.unwrapped.model.dof_frictionloss[1])
        print("Gainprm: ", self.env.unwrapped.model.actuator_gainprm[0][0])
        print("Biasprm: ", self.env.unwrapped.model.actuator_biasprm[0][0])
        print("Force Range: ", self.env.unwrapped.model.actuator_forcerange[0])
        print("Floor Friction: ", self.env.model.geom_friction[1][0])
    ...
    def plot(self, goal_position, actual_position, actual_vel, ID, t, real_goal, real_position, real_velocity, real_t, save_):
        'Save data'
        df = pd.DataFrame({
            't': t,
        ...
        axs[0].plot(real_t, real_goal, label="Real Goal Position", color='k', linestyle='--', linewidth=1)
        axs[0].plot(real_t, real_position, label='Real Actual Position', color='k')
        ...
        axs[1].plot(real_t, real_velocity, label='Velocity', color='b')
...
if __name__ == "__main__":
    'TEST 1: Without object following IHM trajectory'
    # real_record_path = '/Users/qiyangyan/Desktop/FYP/Sim2Real/XM430 Trajectory Without Object/Model_Dynamics_20240310_183242.csv'
    # env = gym.make("VariableFriction-v3")
    # save = False

    'TEST 2: Without object '
    ...
    # real_record_path = '/Users/qiyangyan/Desktop/FYP/Sim2Real/XM430 Multiple IHM-like Trajectory/Set1/Model_Dynamics_20240315_171236_1.csv'
    env = gym.make("VariableFriction-v3", render_mode="human")
    save = False

    'TEST 3: Torque'
    # t, actual_position, actual_vel = sim.torque(2, 0)

    'TEST 4: Calibration Trajectory'
    # real_record_path = '/Users/qiyangyan/Desktop/FYP/Sim2Real/XM430 Calibration IHM-like Trajectory/Model_Dynamics_20240305_181840.csv'
    # env = gym.make("VariableFriction-v3")
    # save = False
    ...
    real_goal = np.array(df['Goal Position'])
    ...
    # ]
    sim = MuJoCo_Simulation_executor(env)
    sim.check_parameter_left()

    for goal_pos_list in trajectory_list:
        t_, sim_present_position_, sim_present_vel_, sim_goal_position_ = sim.manual_policy(ID_, goal_pos_list, real_time[-1])
        sim.plot(sim_goal_position_, sim_present_position_, sim_present_vel_, ID_, t_, real_goal, real_position, real_velocity, real_time, save)
