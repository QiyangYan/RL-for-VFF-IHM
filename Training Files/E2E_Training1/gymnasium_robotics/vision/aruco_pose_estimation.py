...
from gymnasium_robotics.vision.filters import FILTER
...
class ARUCO:
    def __init__(self, quat=False, aruco_type="DICT_4X4_100"):
        ...
        # self.beta = beta  # filter coefficient
        # self.quaternion = np.array([1.0, 0.0, 0.0, 0.0])  # initial quaternion

    def load_camera_params(self, matrix_path="calib/camera_matrix.npy",
                           coef_path="calib/dist_coeffs.npy"):
        script_dir = os.path.dirname(os.path.abspath(__file__))
        matrix_path_full = os.path.join(script_dir, matrix_path)
        coef_path_full = os.path.join(script_dir, coef_path)

        self.intrinsic_camera = np.load(matrix_path_full)
        self.distortion = np.load(coef_path_full)
    ...
    def convert_corner_2_world_coord(euler, object_size, centre):
        ...
        corners = np.array([
            [centre[0] + cross_length * np.cos(np.deg2rad(45 - euler[2])),
             centre[1] - cross_length * np.sin(np.deg2rad(45 - euler[2])),
             centre[2]],
            [centre[0] + cross_length * np.cos(np.deg2rad(45 + euler[2])),
             centre[1] + cross_length * np.sin(np.deg2rad(45 + euler[2])),
             centre[2]],
        ...
        if ids is not None and len(corners) > 0:
            for i, corner in enumerate(corners):
                if ids[i] == 2:  # the aruco marker on base
                    ...
                    # self.rvec_list.append(rvec[0][0])
    ...
    def adjust_corner_order(self, corners, rvec):
        ...
        # Placeholder for logic to determine marker orientation
        # This is an example and might need adjustment based on actual marker orientation
        # You need to replace this with logic based on your application's requirements
        if euler_angles[0] > 45:
            # Assuming this condition indicates a certain orientation of the marker
            # Rotate the corner order accordingly
            # This is just an example condition and action
            corners = np.roll(corners, shift=-1, axis=0)

        # More conditions based on euler_angles to adjust the corner order
        # Ensure the first corner is in the +x and +y region according to the ArUco marker's frame

        return corners

    def pose_estimation_quat(self, display, object_size, aruco_size, object_aruco_id=None, base_aruco_id=2):
        """
        ...
        if ids is not None and len(corners) > 0:
            for i, corner in enumerate(corners):
                if ids[i] == base_aruco_id:  # base pose
                    rvec, tvec, _ = cv2.aruco.estimatePoseSingleMarkers(corner, 0.0335, self.intrinsic_camera,
                                                                        self.distortion)
                    rotation = R.from_rotvec(rvec[0][0])
                    euler = rotation.as_euler('xyz', degrees=True)
                    euler[0] = 180
                    ...
                    rvec, tvec, _ = cv2.aruco.estimatePoseSingleMarkers(corner, aruco_size, self.intrinsic_camera,
                                                                        self.distortion)
                    rotation = R.from_rotvec(rvec[0][0])
                    euler = rotation.as_euler('xyz', degrees=True)
                    euler[0] = 180
        ...
        if centre_pose.any():
            # if len(self.poses) > 0:
            #     print("Check: ", centre_pose)
            #     if abs(centre_pose[1] - self.poses[-1][1]) > 0.05:
            #         print("Outlier ----------------------------- ")
            #         return self.pose_estimation_quat(display=display, object_size=object_size, aruco_size=aruco_size, object_aruco_id=3)

            object_pose = []
            # for idx, item in enumerate(centre_pose):
            #     if len(self.poses) > 0:
            #         if abs(item - self.poses[-1][idx]) > abs(item):
            #             centre_pose[idx] = -item
            ...
            corner_pos = self.convert_corner_2_world_coord(euler_centre, object_size, centre_pose_filter[:3])
            # corner_pos = self.adjust_corner_order(corner_pos, rvec)
            # print("corners: ", corner_pos)
        ...
        if base_pose.any():
            ...
            # if len(self.filter_poses_base) > 10:
            #     mean_x = sum(pose[0] for pose in self.filter_poses_base) / len(self.filter_poses_base)
            #     mean_y = sum(pose[1] for pose in self.filter_poses_base) / len(self.filter_poses_base)
            #     if abs(base_pose_filter[0]-mean_x) <= 0.0004 and abs(base_pose_filter[1]-mean_y) <= 0.0004:
            #         self.filter_poses_base.append(base_pose_filter)  # Append filtered pose
            #         self.poses_base.append(base_pose)  # Append original pose
            #     else:
            #         self.filter_poses_base.append(self.filter_poses_base[-1])  # filtered pose
            #         self.poses_base.append(self.poses_base[-1])  # no filter
            if 0.092 < base_pose_filter[0] < 0.0925:
                self.filter_poses_base.append(base_pose_filter)  # Append filtered pose
                self.poses_base.append(base_pose)  # Append original pose
            elif len(self.filter_poses_base) > 0:
                self.filter_poses_base.append(self.filter_poses_base[-1])  # filtered pose
                self.poses_base.append(self.poses_base[-1])  # no filter
            else:
                self.filter_poses_base.append(base_pose_filter)  # filtered pose
                self.poses_base.append(base_pose)  # no filter
        else:
            if self.poses_base:
                self.filter_poses_base.append(self.filter_poses_base[-1])  # filtered pose
                self.poses_base.append(self.poses_base[-1])  # no filter
        ...
        mean_x = sum(pose[0] for pose in self.filter_poses_base) / len(self.filter_poses_base)
        mean_y = sum(pose[1] for pose in self.filter_poses_base) / len(self.filter_poses_base)
        base = [mean_x, mean_y, 0, 0, 0, 0, 0]

        # return self.frame, self.object_poses[-1], self.poses[-1], self.filter_poses_base[-1]
        # print(self.object_poses[-1], self.poses[-1], base)
        return self.frame, self.object_poses[-1], self.poses[-1], base
        ...
        try:
            while True:
                ...
                if not quat:
                    ...
                    self.pose_estimation_quat(display=display, object_size=object_size, aruco_size=aruco_size, object_aruco_id=3)
                ''' --------------------------------------------------------------- '''
    ...
    def plot_rvec(self):
        time_axis = np.arange(len(self.rvec_list))
        plt.figure(figsize=(4, 4))
        for i in range(3):
            plt.subplot(2, 2, i + 1)
            if i < 3:
                if i == 1:
                    plt.plot(time_axis, abs(np.array(self.rvec_list)[:, i]), label='Pose', color='midnightblue')
                else:
                    plt.plot(time_axis, np.array(self.rvec_list)[:, i], label='Pose', color='midnightblue')
                plt.xlabel('Time/s')
                plt.ylabel(f'Rvec_{i}')
    ...
    def plot_track_real(self, display=True):
        self.plot_track_quat(self.poses, self.object_poses, self.loss_track_history, self.poses_base,
                             self.filter_poses_base, display=display)
        self.plot_quat_scatter(self.object_poses, display=display)

    @staticmethod
    def plot_track_quat(poses, filter_poses, loss_track_history, poses_base, filter_poses_base, display=True):
        """
        This function is used for plot the history of pose estimation process and other information
        ...
        # plt.plot(time_axis, np.array(filter_poses_base)[:, 0] * 100, label='Base - x -filter', color='orange')
    ...
    def plot_quat_scatter(self, object_poses, onlyCentre=False, display=True):
    ...
    def camera_to_base_frame(self, object_poses, base_poses):
        ...
        :return:
        """
        assert len(base_poses) == 7, f"wrong shape, check: {object_poses}"
        base_poses = [0.10136, -0.0039, 0, 0, 0, 0]
        base_camera = np.concatenate([base_poses[:3], np.zeros(4)])  # base aurco in camera frame
        # print("base in camera frame: ", base_camera)
        actuator_height = 0.002

        corner_camera = np.array(object_poses)[1:5]  # object corners in camera frame
        # print(base_camera)
        corner_gripper = corner_camera - base_camera
        corner_gripper[:, 2] = actuator_height

        object_camera = object_poses[0]
        # print(object_camera)
        ...
        aruco_offset_on_base = [-0.018, 0, 0,
                                0, 0, 0, 0]
        # print("Base: ", base_camera, base_camera+[-0.06225, 0.0366, actuator_height, 0, 0, 0, 0])
        # print(np.array([-0.08025, 0.0366, actuator_height, 0, 0, 0, 0])-np.array(aruco_offset_on_base))
        '''With reference to the base aruco marker'''
        env_obs_vision = {
            'left_xm': np.array([-0.08025, 0.0366, actuator_height, 0, 0, 0, 0])-np.array(aruco_offset_on_base),
            'right_xm': np.array([-0.08025, -0.0366, actuator_height, 0, 0, 0, 0])-np.array(aruco_offset_on_base),
            'object_corner': corner_gripper,
            'object_centre': object_gripper,
        }

        env_obs_vision_base = self.swap_xy_axes(env_obs_vision)
        # print("Check: ", env_obs_vision_base)

        return env_obs_vision_base

    def get_obs_aruco(self, object_size, aruco_size, display=False):
        # print(object_size, aruco_size)
        frame, object_pose_filter, object_pose, base_pose = self.pose_estimation_quat(display=display, object_size=object_size, aruco_size=aruco_size, object_aruco_id=3)
        # print(object_pose)
        # print(base_pose)
        obs_dict = self.camera_to_base_frame(object_pose_filter, base_pose)
        # obs_dict = self.camera_to_base_frame(object_pose, base_pose)
        return obs_dict

    def init_camera_for_obs(self, object_size, aruco_size, display=False):
        """
        ...
        if display:
            ...
            cv2.resizeWindow('Frame', 640, 480)  # Set the window size
        # for _ in range(50):
        #     self.get_obs_aruco(object_size=object_size, aruco_size=aruco_size, display=display)
...
if __name__ == "__main__":
    ...
    aruco_obs_dict = aruco_pose.get_obs_aruco(aruco_size=0.0275, object_size=0.03)
    print(aruco_obs_dict)
    # aruco_pose.get_pose(store=False,
    #                     object_size=0.03,
    #                     aruco_size=0.0275,
    #                     display_fps=False,
    #                     auto_terminate=True,
    #                     terminate_length=200,
    #                     display=True,
    #                     record=False,
    #                     show_plot=True,
    #                     quat=True)
