...
class RealEnv(BULK, ARUCO):
    ...
    def step_real(self, action):
        ...
        if gripper_action != self.gripper_pos_ctrl:
            ...
            print("Check: ", self.goal_pos, gripper_action)
    ...
    def get_obs_real(self, display=False):
        ...
        assert len(gripper_obs) == 8, f"Check: {len(gripper_obs)}"
        ...
        assert len(achieved_goal) == 7, f"Check: {len(achieved_goal)}"
        achieved_goal_with_contact_point_radi = self._get_achieved_goal_real(achieved_goal.copy(), corners, left_xm, right_xm)
        object_centre_radi_diff, achieved_goal_centre_radi, desired_goal_centre_radi = self._get_centre_radi_diff_real(achieved_goal.copy(), self.real_env.goal.ravel().copy(), left_xm, right_xm)
        # achieved_goal_euler = self.convert_quat_to_euler(achieved_goal.copy())
        # goal_euler = self.convert_quat_to_euler(self.real_env.goal.ravel().copy()[:7])
        # angle_diff = achieved_goal_euler[5]+90 - goal_euler[3]

        _, angle_diff = self._goal_distance(self.real_env.goal.ravel().copy()[:7], achieved_goal[:7])

        # print(np.deg2rad(angle_diff), angle_diff)

        ' Map from real coordinate to sim coordinate '
        # TODO: Check
        # achieved_goal[1] -= 0.12212
        ...
        observation = np.concatenate(
            [
                gripper_obs,
                achieved_goal_with_contact_point_radi[-2:]
            ]
        )
        ...
        full_observation = np.concatenate(
            [
                gripper_obs,  # (8, )
                achieved_goal,  # (7, )
                object_centre_radi_diff,  # (2, )
                np.array([np.deg2rad(angle_diff)]),  # (1, )
            ]
        )

        # TODO: check object_centre_radi_diff, angle_diff

        if len(self.real_env.goal.ravel()) == 11:
            desired_goal = np.concatenate([self.real_env.goal.ravel().copy()[:7], self.real_env.goal_radi])
        else:
            desired_goal = self.real_env.goal.ravel().copy()

        complete_obs = {
            "observation": observation.copy(),
            "achieved_goal": achieved_goal_with_contact_point_radi.copy(),
            "desired_goal": desired_goal,
        }

        demo_obs = {
            "observation": full_observation.copy(),
            "achieved_goal": np.concatenate(
                [
                    achieved_goal,
                    achieved_goal_centre_radi.copy(),
                ]),
            "desired_goal": np.concatenate(
                [
                    self.real_env.goal.ravel().copy()[:7],
                    desired_goal_centre_radi.copy(),
                ]
            ),
        }

        desired_goal_len = len(demo_obs["desired_goal"])
        assert desired_goal_len == 9, f"Check: {desired_goal_len}"
        ...
        obs = {
            ...
            "corners": corners,
            "demo_obs": demo_obs["observation"].copy(),
            "demo_achieved_goal": demo_obs["achieved_goal"].copy(),
            "demo_desired_goal": demo_obs["desired_goal"].copy(),
        }
    ...
    def _get_centre_radi_diff_real(self, object_qpos, goal_qpos, left_xm, right_xm, display=True):
        """
        7 position element of object + 2 radius, without randomisation
        ...
        achieved_goal_radi = self.real_env.compute_goal_radi(
                                                    ...
                                                    left_motor=left_xm[:3],
                                                    right_motor=right_xm[:3])
        desired_goal_radi = self.real_env.compute_goal_radi(
            b=goal_qpos[:3],
            a=goal_qpos[:3],
            left_motor=left_xm[:3],
            right_motor=right_xm[:3])

        radi_diff_centre = np.array(desired_goal_radi) - np.array(achieved_goal_radi)
        return radi_diff_centre, achieved_goal_radi, desired_goal_radi
    ...
    def start_rotation_real(self, env_goal):
        ...
        if last_distance < 0.003:
            ...
            while True:
                ...
                if distance > last_distance or distance < 0.003:
                    ...
                    rotation_action = abs(self.end_pos_rotation[self.pos_idx] - self.start_pos_rotation[self.pos_idx])
        ...
        return achieved_goal_euler, goal_euler, pose_diff, self.AngleConvert.xm_2_rad(rotation_action), self.pos_idx, next_env_dict
    ...
    def compute_terminate(self, achieved_goal):
        terminate_r_limit = [0.05, 0.15]
        ...
        elif ((not terminate_r_limit[0] < radius_l < terminate_r_limit[1]) or
              (not terminate_r_limit[0] < radius_r < terminate_r_limit[1])):
...
if __name__ == "__main__":
    ...
    # print(Real_HandEnv.get_obs_dynamixel())
