...
class CollectDemos(TrainEvaluateAgent):
    def __init__(self, env_name, render, seed=True, diffusion=False, collect_demo=True, seed_idx=None, real=False):
        super().__init__(
            env_name_=env_name,
            render=render,
            real=real,
    ...
    def step_multi(self, action, episode_dict, plt_dict, num_steps=11):
        for _ in range(num_steps):
            ...
            if r_dict["pos_control_position"] <= 0.03 \
                ...
                break
        return next_env_dict, r_dict, terminated, info_, episode_dict, plt_dict
    ...
    def run_episode_rl_demos(self,
        ...
        if real:
            state, achieved_goal, desired_goal, state_real, achieved_goal_real = self.extract_env_info(env_dict, real=real)
        else:
        ...
        for t in range(20):
            if evaluation:
                ...
                if real:
                    desired_goal_policyInput = desired_goal
                    state_policyInput = state
                    assert len(state_policyInput) == 10, f"Check: {len(state_policyInput)}"
                    assert len(desired_goal_policyInput) == 2, f"Check: {len(desired_goal_policyInput)}"
                else:
            ...
            if real:
                ...
                for _ in range(11):
                    ...
                    next_env_dict = self.real_env.get_obs_real()
                ...
                if not terminated:
                    next_env_dict, r_dict, terminated, info_, episode_dict, plt_dict = self.step_multi(action, episode_dict, plt_dict)
                    self.r_dict = r_dict

            '''Store trajectories'''
            # print("CheckL: ----------- ", state_real[-1])
            if real:
                print(state_real)
                print(self.env.goal.ravel().copy())
                input("Press")
            else:
                print(state)
                print(self.env.goal.ravel().copy())
            ...
            if real:
                episode_dict["state"].append(np.array(state_real).copy())
                episode_dict["action"].append(np.array(action.copy()))
                episode_dict["achieved_goal"].append(np.array(achieved_goal_real.copy()))
            ...
            # print(achieved_goal_real)
            # print(R.from_quat(episode_dict['achieved_goal'][-1][3:7]).as_euler('xyz', degrees=True))
            # input()

            if real:
                next_state, next_achieved_goal, next_desired_goal, next_state_real, next_achieved_goal_real \
                    = self.extract_env_info(next_env_dict, real=real)
            else:
            ...
            if real:
                ...
                achieved_goal_real = next_achieved_goal_real.copy()

            # self.real_env.plot_track_real(display=True)

            if real:
                ''' Real '''
                # TODO: use contact point radius
                if np.mean(np.array(abs(achieved_goal - desired_goal))) < 0.005:
                    # time.sleep(2)
        ...
        if real:
            if slide_success_real:
                ...
                last_friction = friction_state
                self.real_env.change_friction_real(friction_action_2)
                _, _, pose_diff, rotation_action, pos_idx_rotation, env_dict = self.real_env.start_rotation_real(self.env.goal.ravel().copy())
                _, _, _, next_state_real, next_achieved_goal_real \
                    = self.extract_env_info(next_env_dict, real=real)
                self.real_env.reverse_rotation_real()
                action[0] = rotation_action  # rad, angle difference
                action[1] = 5 / 6 - 1 / 12 + 1 / 6 * pos_idx_rotation
                action[1] = action[1] * 2 - 1

                episode_dict["state"].append(
                    np.array(state_real.copy()))  # original obs contains radi info, don't want that
                episode_dict["achieved_goal"].append(np.array(achieved_goal_real.copy()))
                ...
                angle_diff = achieved_goal_euler[5] + 90 - goal_euler[5] % 360
        ...
        if real:
            assert len(next_achieved_goal_real) == 9, f"Check: {next_achieved_goal_real}"
            assert len(next_state_real) == 18, f"Check: {next_state_real}"

            last_achieved_goal_real = next_achieved_goal_real

            episode_dict["desired_goal"] = [last_achieved_goal_real for _ in episode_dict["achieved_goal"]]
            episode_dict['sampled_desired_goal'] = [desired_goal for _ in episode_dict["achieved_goal"]]
            episode_dict["terminals"][-1] = 1
            episode_dict["next_state"] = episode_dict["state"][1:]
            episode_dict["next_state"].append(next_state_real)
            ...
            assert len(next_state) == 18, f"Check: {next_state}"
        ...
        print(f"------------------------------------{t}")
        # print(episode_dict['desired_goal'])
        # print(episode_dict['state'])
        # input("Press")
        '''Plot IHM trajectory'''
        if real:
            # self.real_env.plot_track_real(display=True)
            self.plot_track(plt_dict["achieved_goal"], display=True, onlyCentre=True)
        else:
            self.plot_track(plt_dict["achieved_goal"], display=True, onlyCentre=True)
    ...
    def collect_demos_with_rotation(self,
        ...
        while True:
            ...
            reset = True if keep_reset is True or ep == 0 or success[-1] == 0 else False
            # reset = False
            ...
            rot_diff = np.array(episode_dict['state'])
            plt.plot(rot_diff[:, -1])
            plt.show()
            ...
            if status == "Failed" or success[-1] == 0:
                ...
                store = (user_input == "" or user_input.lower() != "no")
                if not store:
                    print("\033[91m-\033[0m"*50)
                    print("\033[91m | Not save \033[0m")
                    print("\033[91m-\033[0m"*50)
                    ...
                    if randomisation:
                        ...
                        print(f"\033[92m | Number of Stored Episode: {ep} \033[0m")
                        print("\033[92m-\033[0m" * 50)
            ...
            if (ep + 1) % 10 == 0:
                ...
                if not test and real is False:
                    self.store_demonstration(demonstration_dict, demonstration_file_name)
            elif ep > 0 and test is False and real is True:
                self.store_demonstration(demonstration_dict, demonstration_file_name)
    ...
    def compute_orientation_diff(self, goal_a, goal_b):
        ...
        d_rot = np.deg2rad(R.from_quat(quat_a).as_euler('xyz', degrees=True)[0] % 360 - R.from_quat(quat_b).as_euler('xyz',
                                                                                                       degrees=True)[0] % 360)
        print(np.rad2deg(d_rot))
    ...
    def plot_track(self, object_pose, display, onlyCentre):
        self.plot_quat_scatter(object_pose, onlyCentre=onlyCentre, display=display)
    ...
    def plot_quat_scatter(self, object_poses, onlyCentre=False, display=True):
        ...
        if not onlyCentre:
            ...
            for i, data in enumerate(object_poses):
                plt.scatter(data[0], data[1], s=50, zorder=5, color='red', label='Center' if i == 0 else "")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--test", action='store_true', help="do test, don't save anything")
    parser.add_argument("--real", action='store_true', help="Run demo collection in real")
    parser.add_argument("--no_keep_reset", action='store_false', help="Don't keep reset environment when terminates")
    ...
    print("keep reset: ", args.no_keep_reset)
